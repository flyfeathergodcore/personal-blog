...
public:
    ...
    // ── Lambda / 函数式注册 ──
    // 接收任何可调用对象 `Response(const Context&)`，自动包装为 FunctionHandler。
    // SFINAE 约束（is_invocable_r_v）确保仅对可调用对象生效，不劫持上面的
    ...
    void Add(std::string path, Fn fn) {
        Add(std::move(path),
            std::unique_ptr<RequestHandler>(
                std::make_unique<FunctionHandler>(std::move(fn))));
    }
    template<typename Fn,
        std::enable_if_t<std::is_invocable_r_v<Response, Fn, const Context&>, int> = 0>
    ...
    /// @param params optional capture of path parameters (views into @a path)
    RequestHandler* Match(std::string_view path,
                          std::vector<std::pair<
                              std::string_view,
...
private:
    ...
    /// 返回的 view 仍指向调用者 buffer（只是缩短），捕获的路径参数不受影响。
    static std::string_view StripQuery(std::string_view path);
};
