...
#include "http/server/ws_relay.hpp"
...
static coro::Task<bool> ReadChunkedBody(tcp::Stream& reader,
                                        std::string& out)
{
    for (;;) {
        std::string line;
...
// 读 101 头时把紧随其后的首帧字节留在 Stream 缓冲中，导致后续读取器漏读。
...
struct FrameSource {
    ...
    coro::Task<tcp::IoResult> read_some(void* buf, size_t n,
                                        int64_t timeout_ms = -1) {
        if (pos < pending.size()) {
            size_t take = std::min(n, pending.size() - pos);
            std::memcpy(buf, pending.data() + pos, take);
            pos += take;
            co_return tcp::IoResult{take, tcp::IoError::None};
        }
...
coro::Task<Response> ReverseProxy::HandleAsync(const Context& ctx)
{
    ...
    // ── 选上游 ──
    if (const auto* upstream = pool_->Pick()) {
        // ── 转发并组装响应 ──
    ...
    unsigned short port)
{
    ...
    if (!region) co_return Response::Raw(502, R"({"error":"Bad Gateway"})");

    auto& conn_pool = UpstreamConnPool::Instance();
    auto host_str   = std::string(host);
    auto port_str   = std::to_string(port);

    // ── 建连辅助：tcp::Connect 内部 resolve + 非阻塞 connect ──
    auto make_conn = [&]() -> coro::Task<std::unique_ptr<tcp::Stream>> {
        return tcp::Connect(host_str, port, 10000);
    };

    // ── 尝试取池化连接 ──
    auto pooled = conn_pool.Acquire(host_str, port);
    std::unique_ptr<tcp::Stream> sock;
    bool from_pool = false;

    if (pooled) {
        sock = std::make_unique<tcp::Stream>(std::move(pooled->socket));
        from_pool = true;
    }
    ...
    const Context& ctx, WsConnectionBase& client_conn)
{
    ...
    // ── 建连上游 ──
    auto upstream_sock = co_await tcp::Connect(host_str, upstream->port, 10000);
    ...
    std::string status_line;
    auto r1 = co_await upstream_sock->read_until("\r\n", status_line);
    if (!r1.ok()) { pool_->ReportFailure(upstream); co_return; }

    std::string hdr_block;
    auto r2 = co_await upstream_sock->read_until("\r\n\r\n", hdr_block);
    ...
    if (status_line.find("101") == std::string::npos) {
        ...
        std::string err_body;
        if (fr.chunked) {
            if (!(co_await ReadChunkedBody(*upstream_sock, err_body))) { /* 忽略读错误 */ }
        } else if (fr.has_content_length) {
            auto rb = co_await upstream_sock->read_exact(fr.content_length, err_body);
            (void)rb;
        } else {
            if (fr.upstream_close) {
    ...
    FrameSource<tcp::Stream> upstream_src{
        upstream_sock->take_buffered(), 0, *upstream_sock};
