...
#include "http/server/h2_stream_processor.hpp"
#include "http/protocol/region_pool.hpp"
#include "http/server/sse_push.hpp"
...
void H2Session::OnData(const H2FrameHeader& hdr, const uint8_t* payload)
{
    ...
    const auto actual_len = hdr.length;
    const auto state = stream_mgr_.GetState(sid);
    const bool previously_closed = state == H2StreamState::Closed
        || state == H2StreamState::HalfClosedRemote
        || (state == H2StreamState::Idle
            && (sid & 1) != 0
            && sid <= stream_mgr_.LastClientStreamId());

    // DATA can remain in flight after END_STREAM/RST_STREAM. It no longer has
    // a usable stream window, but it still consumes the connection window.
    // Return that connection credit immediately so rejected streams cannot
    // drain unrelated streams, and never recreate the removed stream ledger.
    if (previously_closed) {
        if (actual_len > flow_control_.RecvWindow(0)) {
            WriteGoAway(stream_mgr_.LastClientStreamId(), H2Error::FLOW_CONTROL_ERROR);
            goaway_sent_ = true;
            return;
        }
        flow_control_.ConsumeConnectionRecv(actual_len);
        const auto credit = flow_control_.PopCredit(0);
        if (credit != 0)
            WriteWindowUpdate(0, credit);

        if (state == H2StreamState::HalfClosedRemote) {
    ...
    // DATA on a genuinely idle stream is a connection protocol error.
    ...
    if (ctx.ws_active_) {
        ...
        // Check before appending so an oversized frame is never temporarily
        // allocated. The subtraction form also avoids size_t addition overflow.
        const auto body_size = ctx.Body().size();
        const bool body_too_large = max_body_size_ > 0
            && (ctx.ContentLength() > max_body_size_
                || body_size > max_body_size_
                || data_len > max_body_size_ - body_size);
        if (body_too_large) {
            WriteRstStream(sid, H2Error::CANCEL);
            ctx.stream_closed_ = true;
            stream_mgr_.OnStreamClose(sid);

            // Do not replenish the dead stream. Its DATA was consumed from the
            // socket, so return all pending connection credit immediately.
            const auto credit = flow_control_.PopCredit(0);
            if (credit != 0)
                WriteWindowUpdate(0, credit);
        ...
        // Normal mode — accumulate body only after the limit check.
