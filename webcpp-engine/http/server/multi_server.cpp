...
void MultiServer::Start()
{
    ...
    for (int i = 0; i < cfg_.threads; ++i) {
        ...
        // tcp::Listener::open 内部：socket() → setsockopt(SO_REUSEPORT|SO_REUSEADDR) → bind → listen
...
coro::Task<void> MultiServer::Listen(Worker& w, int wid)
{
    while (!shutdown_) {
        ...
        if (cfg_.tls_port > 0) {
            auto h = HandleTls(w, wid, std::move(tcp));
            w.loop.post(h.handle());
        } else {
            auto h = HandlePlain(w, wid, std::move(tcp));
            w.loop.post(h.handle());
        }
    }
...
coro::Task<void> MultiServer::HandleTls(Worker& w, int wid, tcp::Stream tcp)
{
...
// 参数：w - 所属 worker；wid - worker 序号；tcp - 已 accept 的 TCP 流
coro::Task<void> MultiServer::HandlePlain(Worker& w, int wid, tcp::Stream tcp)
{
