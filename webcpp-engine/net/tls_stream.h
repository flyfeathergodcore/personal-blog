// net 层 TLS 流：封装 tcp::Stream + SSL（服务端模式），协程式非阻塞握手/读写。
...
public:
    ...
    TlsStream(tcp::Stream tcp, SSL_CTX* ctx);
    // 析构：关闭 TLS 连接与底层 fd
    ~TlsStream();
    ...
    coro::Task<tcp::IoResult> handshake(int64_t timeout_ms = 10000);
    // 解密读取一帧，语义与 tcp::Stream::read_some 一致（Eof=对端 close_notify）
    coro::Task<tcp::IoResult> read_some(void* buf, size_t n, int64_t timeout_ms = -1);
    ...
    // 合并无 syscall 收益；此接口仅为与 tcp::Stream 的模板接口对齐）。
