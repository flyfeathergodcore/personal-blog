...
ParseResult H1Parser::Feed(const char* data, size_t len)
{
    ...
    ClearParams();    // 路径参数按请求注入，keep-alive 复用必须每请求清
    ClearQuery();     // query 惰性缓存按请求重建
...
void H1Parser::ResetMessage()
{
    ...
    // 注意不清 h2_detected_：那是连接级标志，仅 H1Parser::Reset()（复用前）清零。
    state_ = REQUEST_LINE;
    line_len_ = 0;
    header_count_ = 0;
    content_length_ = 0;
...
void H1Parser::Reset()
{
    ...
    // 防止上一连接在请求中途断开（state_ 停在 HEADERS/BODY）时污染新请求。
    ResetMessage();
    h2_detected_ = false;
}
